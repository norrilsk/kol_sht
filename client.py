...
import cv2 as cv
from recognizer import get_key_from_print
...
class User:
    def __init__(self, bio=0, login= ""):
        ...
        if (login):
            try:
                self.autorize(str(bio),login)
            except:
                self.register(login,str(bio))
                self.autorize(str(bio), login)
    ...
    def diffi_with_user(self, user):
        self._a[user] = random.randint(2, 2 ** 64) + self._bio
        self._A[user] = fast_power(self._g, self._a[user], self._p)
        self.send_message_to_user(user,
                                  "razrabotchyk_pidor90585355972355049027130019104418304977474815248020730362080159511218759452819 " + str(
                                      self._A[user]), from_diffi=True)
    ...
    def createParser(self):
        ...
        parser.add_argument('-receive', '-recv', action='store_const', const=True,
                            help="recieve all messages from the server")
...
if __name__ == '__main__':
    ...
    user = None
    if len(sys.argv) > 2:
        PATH_TO_PICTURE = sys.argv[2]
        login = sys.argv[1]
        img00 = cv.imread(PATH_TO_PICTURE, cv.IMREAD_GRAYSCALE)
        password = get_key_from_print(img00)
        print("generated password = ", password)
        user = User(login =login, bio = password)
    else:
        user = User()
    exit = False
    while not (exit):
